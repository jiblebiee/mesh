...
//        println("smile322 ===> ${msg}")
//msg0322        println("State ${state}, Usage: ${usage}")

        // This function called when receive click event or something.
        if ((state != 2) || (msg.size < 2)) return;
//        println("Tunnel-onBinaryMessage: ${msg.size}, ${msg.toByteArray()}")
//        println("Tunnel-onBinaryMessage : ${msg[0].toInt()}")
        try {
            if (msg[0].toInt() == 123) {
...
//        println("processBinaryDesktopCmd ${cmd} > ${cmdsize} > ${msg} > ${msg[0]} ${msg[1]} ${msg[2]} ${msg[3]} ${msg[4]} ${msg[5]} ${msg[6]} ${msg[7]} ${msg[8]} ${msg[9]}")
        ...
        when (cmd) {
            ...
            2 -> { // Mouse input
                ...
                if (cmdsize == 10 || cmdsize == 12) {

                    x = ((msg[6].toInt() shl 8) or (msg[7].toInt() and 0xFF))
